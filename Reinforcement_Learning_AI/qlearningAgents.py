...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        self.qValues = util.Counter()   # (state, action) to float
    ...
    def computeActionFromQValues(self, state):
        ...
        # Max Q and best actions
        values = [(a, self.getQValue(state, a)) for a in actions]
        bestVal = max(v for _, v in values)
    ...
    def update(self, state, action, nextState, reward):
        ...
        nextV = self.computeValueFromQValues(nextState)  # 0 on terminal
        target = reward + self.discount * nextV
        self.qValues[(state, action)] = (1 - self.alpha) * oldQ + self.alpha * target
